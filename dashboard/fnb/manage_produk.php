...
session_start();
    ...
    if(isset($_GET['del'])) {
        $id=intval($_GET['del']);
        $adn="DELETE from produk where id=?";
                            ...
                            <div class="card-body">
                                ...
                                <div class="table-responsive">
                                    ...
                                    <thead class="thead-dark">
                                        ...
                                        FROM produk_fb_masuk AS m
                                        LEFT JOIN (
                                            SELECT kode_produk, SUM(jumlah_masuk) AS total_masuk
                                            FROM produk_fb_masuk
                                            GROUP BY kode_produk
                                        ) AS t ON m.kode_produk = t.kode_produk
                                        LEFT JOIN (
                                            SELECT p.kode_produk, (SUM(p.jumlah_keluar) + COALESCE((SELECT SUM(jumlah_keluar) FROM produk_fb_keluar WHERE kode_produk = p.kode_produk AND tanggal_keluar < p.tanggal_keluar), 0)) AS total_keluar, p.tanggal_keluar
                                            FROM produk_fb_keluar AS p
                                            INNER JOIN (
                                                SELECT kode_produk, MAX(tanggal_keluar) AS max_tanggal
                                                FROM produk_fb_keluar
                                                GROUP BY kode_produk
                                        ...
                                        <td><?php echo $row->tanggal;?></td>                                        
                                            ...
                                            <?php
                                            $check_query = "SELECT COUNT(*) as count FROM produk WHERE kode_produk = ?";
                                            ...
                                            if ($data['count'] == 0) {
                                                ...
                                                $insert_query = "INSERT INTO produk (kode_produk, nama_produk, kuantitas, kategori, satuan, tanggal) 
                                                                 VALUES (?, ?, ?, ?, ?, ?)";
                                                ...
                                                $update_query = "UPDATE produk SET nama_produk = ?, kuantitas = ?, kategori = ?, satuan = ?, tanggal = ? 
                                                                 WHERE kode_produk = ?";
                                        ...
                                        $cnt=$cnt+1;
                                            ...
                                            $delete_query = "DELETE FROM produk WHERE kode_produk NOT IN (SELECT kode_produk FROM produk_fb_masuk)";
